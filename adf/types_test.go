...
func TestADFDocumentString(t *testing.T) {
	...
	str := doc.String()
	expected := "Document{Version: 1, Type: doc, Content: 1 nodes}"

	if str != expected {
		t.Errorf("Document.String() = %s, want %s", str, expected)
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if str != tt.contains && len(str) < len(tt.contains) {
				t.Errorf("Node.String() = %s, should contain %s", str, tt.contains)
			}
		})
	}
}
