...
func (p *MarkdownParser) conversionContext() ConversionContext {
	return ConversionContext{
		...
		Registry:           p.registry,
		ParseNested: func(lines []string, nestedLevel int) ([]Node, error) {
...
func (p *MarkdownParser) ParseMarkdownToADFNodes(lines []string) ([]Node, error) {
	var result []Node
...
func (p *MarkdownParser) parseNext(lines []string) (*Node, int, error) {
	if len(lines) == 0 {
		return nil, 1, nil
	}
