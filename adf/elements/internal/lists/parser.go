...
func ParseBulletList(markdown string, manager placeholder.Manager) (adf.Node, error) {
	...
	list := doc.FirstChild()
	if list == nil {
		return adf.Node{}, fmt.Errorf("no list found in markdown")
	}

	// Verify it's a List node
	listNode, ok := list.(*ast.List)
	if !ok {
		return adf.Node{}, fmt.Errorf("expected List node, got %T", list)
	}

	// Verify it's an unordered list
	if listNode.IsOrdered() {
		return adf.Node{}, fmt.Errorf("expected bullet list, got ordered list")
	}
...
// The manager parameter is used to restore placeholder nodes (e.g., emoji, mentions) during conversion.
func ParseOrderedList(markdown string, manager placeholder.Manager) (adf.Node, error) {
	// Parse markdown with goldmark
	source := []byte(markdown)
	parser := goldmark.New()
	doc := parser.Parser().Parse(text.NewReader(source))
	...
	if !listNode.IsOrdered() {
		return adf.Node{}, fmt.Errorf("expected ordered list, got bullet list")
	}
...
func ConvertListNode(list *ast.List, source []byte, manager placeholder.Manager) (adf.Node, error) {
...
func convertList(list *ast.List, source []byte, manager placeholder.Manager) (adf.Node, error) {
	// Determine list type
	listType := adf.NodeTypeBulletList
	if list.IsOrdered() {
		listType = adf.NodeTypeOrderedList
	}
	...
	if list.IsOrdered() && list.Start != 1 {
		attrs = map[string]any{"order": float64(list.Start)}
	}
...
func convertListItem(listItem *ast.ListItem, source []byte, manager placeholder.Manager) (adf.Node, error) {
...
func convertHTMLBlock(htmlBlock *ast.HTMLBlock, source []byte, manager placeholder.Manager) (adf.Node, error) {
...
func convertTextBlock(textBlock *ast.TextBlock, source []byte, manager placeholder.Manager) (adf.Node, error) {
	...
	for i := 0; i < textBlock.Lines().Len(); i++ {
		...
		if i < textBlock.Lines().Len()-1 {
			allContent = append(allContent, adf.Node{
