...
func isBlockBoundary(trimmed string, registry adf.Registry) bool {
	if registry == nil {
		return false
	}
...
func (pc *paragraphConverter) ToMarkdown(node adf.Node, context adf.ConversionContext) (adf.EnhancedConversionResult, error) {
	...
	var renderableContent []adf.Node
	for _, child := range node.Content {
		isUnknownInline := !adf.IsInlineNode(child.Type)
...
func (pc *paragraphConverter) FromMarkdown(lines []string, startIndex int, context adf.ConversionContext) (adf.Node, int, error) {
	...
	if err != nil {
		return adf.Node{}, 0, fmt.Errorf("failed to parse inline content: %w", err)
	}
