...
func (tc *tableRenderer) ToMarkdown(node adf.Node, context adf.ConversionContext) (adf.RenderResult, error) {
	...
	if context.PreserveAttrs && len(nonDefaultAttrs) > 0 {
		...
		if err != nil {
			return convresult.CreateErrorResult(err.Error(), adf.MarkdownTable), err
		}
	...
	result := convresult.CreateSuccessResult(finalMarkdown, adf.MarkdownTable)
...
func (tc *tableRenderer) parseXMLWrappedTable(lines []string) (adf.Node, error) {
	...
	if startIdx == -1 || endIdx == -1 {
		return adf.Node{}, fmt.Errorf("malformed XML table: missing opening or closing tag")
	}
	...
	if err != nil {
		return adf.Node{}, fmt.Errorf("failed to parse markdown table content: %w", err)
	}
