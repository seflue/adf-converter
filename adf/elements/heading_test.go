...
func TestHeadingConverter_ToMarkdown(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := hc.ToMarkdown(tt.node, adf.ConversionContext{Registry: newTestRegistry()})
...
func TestHeadingConverter_FromMarkdown(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			node, consumed, err := hc.FromMarkdown(tt.lines, tt.startIndex, adf.ConversionContext{Registry: newTestRegistry()})
			if (err != nil) != tt.wantErr {
			...
			if node.Type != adf.NodeTypeHeading {
				t.Errorf("FromMarkdown() node type = %v, want %v", node.Type, adf.NodeTypeHeading)
			}
...
func TestHeadingConverter_RoundTrip(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			node, _, err := hc.FromMarkdown(lines, 0, adf.ConversionContext{Registry: newTestRegistry()})
			if err != nil {
			...
			result, err := hc.ToMarkdown(node, adf.ConversionContext{Registry: newTestRegistry()})
			if err != nil {
...
func TestHeadingConverter_FromMarkdown_MarksPreserved(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			line: "## Intro **bold** end",
			wantNodes: []adf.Node{
				{Type: adf.NodeTypeText, Text: "Intro "},
				{
					Type:  adf.NodeTypeText,
					Text:  "bold",
					Marks: []adf.Mark{{Type: "strong"}},
				},
				{Type: adf.NodeTypeText, Text: " end"},
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			node, consumed, err := hc.FromMarkdown([]string{tt.line}, 0, adf.ConversionContext{Registry: newTestRegistry()})
			if err != nil {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			node := adf.Node{
				...
				Content: []adf.Node{
					{Type: adf.NodeTypeText, Text: "Text"},
				},
			}
			result, err := hc.ToMarkdown(node, adf.ConversionContext{Registry: newTestRegistry()})
			if err != nil {
