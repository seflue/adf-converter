...
// testFromMarkdown wraps DefaultConverter.FromMarkdown and discards the deletion
// summary to match the legacy top-level FromMarkdown return shape used in tests.
func testFromMarkdown(markdown string, session *placeholder.EditSession, manager placeholder.Manager, registry *adf.ConverterRegistry) (adf.Document, error) {
	...
	doc, _, err := c.FromMarkdown(markdown, session)
	return doc, err
}
