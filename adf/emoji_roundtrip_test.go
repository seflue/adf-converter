...
func TestEmojiRoundTrip(t *testing.T) {
	...
	markdown, session, err := testToMarkdown(originalADF, classifier, manager, defaults.NewRegistry())
	...
	reconvertedADF, err := testFromMarkdown(markdown, session, manager, defaults.NewRegistry())
	...
	require.Equal(t, adf.NodeTypeBulletList, bulletList.Type, "Should be bulletList")
	require.Len(t, bulletList.Content, 1, "BulletList should have 1 listItem")

	listItem := bulletList.Content[0]
	require.Equal(t, adf.NodeTypeListItem, listItem.Type, "Should be listItem")
	...
	require.Equal(t, adf.NodeTypeParagraph, para.Type, "Should be paragraph")
	...
	assert.Equal(t, adf.NodeTypeEmoji, emojiNode.Type, "First node should be emoji (detected from unicode)")
	...
	for _, node := range para.Content[1:] {
		if node.Type == adf.NodeTypeText && node.Text == "Task completed successfully" {
			hasText = true
			require.Len(t, node.Marks, 1, "Text should have code mark")
			assert.Equal(t, adf.MarkTypeCode, node.Marks[0].Type, "Should have code mark")
			break
		}
	}
...
func TestEmojiRoundTrip_MultipleEmojis(t *testing.T) {
	...
	assert.Equal(t, adf.NodeTypeEmoji, para1.Content[0].Type, "First item should have emoji")
	...
	assert.Equal(t, adf.NodeTypeEmoji, para2.Content[0].Type, "Second item should have emoji")
}
