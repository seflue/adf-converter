...
- [Link with &amp; entities](https://example.com/page?a=1&amp;b=2)`,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// Try conversion
			doc, err := testFromMarkdown(test.inputMarkdown, session, manager, defaults.NewRegistry())
...
func TestEdgeCases_EmptyInputs(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			doc, err := testFromMarkdown(test.markdown, session, manager, defaults.NewRegistry())
...
func TestEdgeCases_MalformedADF(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			adf: adf.Document{
				Version: 1,
				Type:    "doc",
				Content: []adf.Node{}, // Empty content
			},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			markdown, session, err := testToMarkdown(test.adf, classifier, manager, defaults.NewRegistry())
			...
			if err == nil && markdown != "" {
				_, err2 := testFromMarkdown(markdown, session, manager, defaults.NewRegistry())
...
func TestEdgeCases_DeeplyNestedStructures(t *testing.T) {
						...
						Content: []adf.Node{
							{
								Type: adf.NodeTypeParagraph,
								Content: []adf.Node{
									{Type: adf.NodeTypeText, Text: "Level 1"},
								},
							},
							{
								Type: adf.NodeTypeBulletList,
										...
										Content: []adf.Node{
											{
												Type: adf.NodeTypeParagraph,
												Content: []adf.Node{
													{Type: adf.NodeTypeText, Text: "Level 2"},
												},
											},
											{
												Type: adf.NodeTypeBulletList,
														...
														Content: []adf.Node{
															{
																Type: adf.NodeTypeParagraph,
																Content: []adf.Node{
																	{Type: adf.NodeTypeText, Text: "Level 3"},
																},
															},
														},
													},
	...
	// Convert to markdown
	markdown, session, err := testToMarkdown(deeplyNestedList, classifier, manager, defaults.NewRegistry())
	...
	assert.Equal(t, adf.NodeTypeBulletList, restored.Content[0].Type)
}
...
func TestEdgeCases_LargeContent(t *testing.T) {
		...
		Content: []adf.Node{
			{
				Type: adf.NodeTypeParagraph,
				Content: []adf.Node{
					{Type: adf.NodeTypeText, Text: longText},
				},
			},
		},
	}
	...
	// Convert to markdown
	markdown, session, err := testToMarkdown(largeParagraph, classifier, manager, defaults.NewRegistry())
...
> Back to first level`
	...
	// Convert to ADF
	doc, err := testFromMarkdown(nestedBlockquoteMarkdown, session, manager, defaults.NewRegistry())
...
- [ ] Another unchecked task`
	...
	// Convert to ADF
	doc, err := testFromMarkdown(taskListMarkdown, session, manager, defaults.NewRegistry())
...
func TestPlainMarkdownTaskList_ParsedAsTaskList(t *testing.T) {
	...
	doc, err := testFromMarkdown(markdown, session, manager, defaults.NewRegistry())
	require.NoError(t, err)
...
Mixed: "Smart quotes" and 'single quotes' and —em dash— and –en dash–`
	...
	// Convert to ADF
	doc, err := testFromMarkdown(specialCharsMarkdown, session, manager, defaults.NewRegistry())
...
Final paragraph.`
	...
	doc, err := testFromMarkdown(mixedValidInvalidMarkdown, session, manager, defaults.NewRegistry())
