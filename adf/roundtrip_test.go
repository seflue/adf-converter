...
func TestRoundTripConversion_BasicDocument(t *testing.T) {
	...
	markdown, restored, err := adf.ConvertRoundTrip(conv, original)
	if err != nil {
...
func TestRoundTripConversion_UnderlineBoldText(t *testing.T) {
	...
	require.Equal(t, adf.NodeTypeParagraph, restored.Content[0].Type)
	...
	assert.True(t, markTypes[adf.MarkTypeStrong], "strong mark must survive roundtrip")
	assert.True(t, markTypes[adf.MarkTypeUnderline], "underline mark must survive roundtrip")
}
...
func TestRoundTripConversion_TextColorBoldText(t *testing.T) {
	...
	assert.True(t, markTypes[adf.MarkTypeStrong], "strong mark must survive roundtrip")
	assert.True(t, markTypes[adf.MarkTypeTextColor], "textColor mark must survive roundtrip")

	for _, m := range restoredNode.Marks {
		if m.Type == adf.MarkTypeTextColor {
...
func TestRoundTripConversion_BoldSubscriptText(t *testing.T) {
	...
	assert.True(t, markTypes[adf.MarkTypeStrong], "strong mark must survive roundtrip")
	assert.True(t, markTypes[adf.MarkTypeSubsup], "subsup mark must survive roundtrip")
}
...
func TestExpandRoundtrip_BasicExpandElement(t *testing.T) {
	...
	assert.Equal(t, original.Version, restored.Version)
	assert.Equal(t, len(original.Content), len(restored.Content))
	assert.Equal(t, adf.NodeTypeExpand, restored.Content[0].Type)
...
func TestRoundTrip_ComplexDocument(t *testing.T) {
	...
	assert.Equal(t, adf.NodeTypeHeading, restored.Content[0].Type)
	assert.Equal(t, adf.NodeTypeParagraph, restored.Content[1].Type)
	assert.Equal(t, adf.NodeTypeBulletList, restored.Content[2].Type)
}
				...
				Content: []adf.Node{
					{
						Type: adf.NodeTypeParagraph,
						Content: []adf.Node{
							{Type: adf.NodeTypeText, Text: "Simple text"},
						},
					},
				},
			},
				...
				Content: []adf.Node{
					{
						...
						Content: []adf.Node{
							{Type: adf.NodeTypeText, Text: "Level 2 Heading"},
						},
					},
				},
			},
...
func TestExpandBackwardCompatibility_DetailsFormat(t *testing.T) {
	...
	assert.Equal(t, adf.NodeTypeExpand, expandNode.Type)
...
func TestExpandNested_NoBlankLineBeforeInnerDetails(t *testing.T) {
	...
	for _, child := range expandNode.Content {
		if child.Type == adf.NodeTypeParagraph {
			foundParagraph = true
		}
		if child.Type == adf.NodeTypeNestedExpand {
...
func TestExpandRoundtrip_NestedExpandWithContent(t *testing.T) {
				...
				Content: []adf.Node{
					{
						Type: adf.NodeTypeParagraph,
						Content: []adf.Node{
							{Type: adf.NodeTypeText, Text: "Text before nested"},
						},
					},
					{
						...
						Content: []adf.Node{
							{
								Type: adf.NodeTypeParagraph,
								Content: []adf.Node{
									{Type: adf.NodeTypeText, Text: "Nested content"},
								},
							},
						},
					},
	...
	assert.Equal(t, adf.NodeTypeExpand, expand.Type)
...
func TestExpandParsing_BackToBackExpands(t *testing.T) {
	...
	assert.Equal(t, adf.NodeTypeExpand, restored.Content[1].Type)
						...
						Content: []adf.Node{
							{
								Type: adf.NodeTypeParagraph,
								Content: []adf.Node{
									{Type: adf.NodeTypeText, Text: "Content with ID"},
								},
							},
						},
					},
						...
						Content: []adf.Node{
							{
								Type: adf.NodeTypeParagraph,
								Content: []adf.Node{
									{Type: adf.NodeTypeText, Text: "Full featured content"},
								},
							},
						},
					},
...
Some **bold text** here.
	...
	assert.Contains(t, types, adf.NodeTypeHeading, "expand content should include heading")
	assert.Contains(t, types, adf.NodeTypeBulletList, "expand content should include bulletList")
}
...
func TestRoundTrip_OrderedList_StartNumber(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
						...
						Content: []adf.Node{
							{
								...
								Content: []adf.Node{
									{
										Type:    adf.NodeTypeParagraph,
										Content: []adf.Node{{Type: adf.NodeTypeText, Text: "Alpha"}},
									},
								},
							},
							{
								...
								Content: []adf.Node{
									{
										Type:    adf.NodeTypeParagraph,
										Content: []adf.Node{{Type: adf.NodeTypeText, Text: "Beta"}},
									},
								},
							},
						},
			...
			assert.Equal(t, adf.NodeTypeOrderedList, list.Type)
		...
		Content: []adf.Node{
			{
				...
				Content: []adf.Node{
					{Type: "tableRow", Content: []adf.Node{
						{Type: "tableHeader", Content: []adf.Node{
							{Type: "paragraph", Content: []adf.Node{
								{Type: "text", Text: "Feature", Marks: []adf.Mark{
									{Type: "strong"},
								}},
							}},
						}},
					...
					{Type: "tableRow", Content: []adf.Node{
						...
						{Type: "tableCell", Content: []adf.Node{
							{Type: "paragraph", Content: []adf.Node{
								{Type: "text", Text: "Done", Marks: []adf.Mark{
									{Type: "em"},
								}},
							}},
						}},
...
func TestRoundTripConversion_InlineCardDataOnly(t *testing.T) {
	...
	card := para.Content[0]
	assert.Equal(t, adf.NodeTypeInlineCard, card.Type)
...
func TestRoundTrip_MediaInline_InParagraph(t *testing.T) {
		...
		Content: []adf.Node{
			{
				Type: adf.NodeTypeParagraph,
				Content: []adf.Node{
					adf.NewTextNode("See attachment: "),
					mediaInlineNode,
					adf.NewTextNode(" for details."),
				},
			},
		},
	}
	...
	conv := defaults.NewDefaultConverter()
	md, restored, err := adf.ConvertRoundTrip(conv, original)
	require.NoError(t, err)
	...
	assert.Equal(t, adf.NodeTypeText, para.Content[0].Type)
	assert.Equal(t, "See attachment: ", para.Content[0].Text)

	assert.Equal(t, adf.NodeTypeMediaInline, para.Content[1].Type)
	...
	assert.Equal(t, adf.NodeTypeText, para.Content[2].Type)
...
func TestRoundTrip_MediaInline_StandaloneInParagraph(t *testing.T) {
	...
	conv := defaults.NewDefaultConverter()
	_, restored, err := adf.ConvertRoundTrip(conv, original)
	require.NoError(t, err)
	...
	assert.Equal(t, adf.NodeTypeParagraph, para.Type)
	require.Len(t, para.Content, 1)
	assert.Equal(t, adf.NodeTypeMediaInline, para.Content[0].Type)
