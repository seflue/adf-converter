...
func TestBlockquoteConverter_FromMarkdown(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			startIndex:       0,
			ctx:              converter.ConversionContext{Registry: newTestRegistry(), PreserveAttrs: false},
		...
		},
		{
			...
			startIndex:       0,
			ctx:              converter.ConversionContext{Registry: newTestRegistry(), PreserveAttrs: true},
			expectedType:     "blockquote",
			expectedContent:  1,
			expectedConsumed: 3,
...
func TestBlockquoteConverter_FromMarkdown_NestedPrefix(t *testing.T) {
	...
	node, consumed, err := bc.FromMarkdown(lines, 0, converter.ConversionContext{Registry: newTestRegistry()})
	require.NoError(t, err)
	assert.Equal(t, 3, consumed)
	assert.Equal(t, "blockquote", node.Type)
