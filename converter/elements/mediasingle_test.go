...
func externalMediaNode(layout, url, alt string) adf_types.ADFNode {
	...
	return adf_types.ADFNode{
		...
		Content: []adf_types.ADFNode{
			{Type: adf_types.NodeTypeMedia, Attrs: mediaAttrs},
		},
	}
}
...
func TestMediaSingleConverter_ToMarkdown(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := mc.ToMarkdown(tt.node, converter.ConversionContext{Registry: newTestRegistry()})
			if tt.wantErr {
				require.Error(t, err)
				return
			}
...
func TestMediaSingleConverter_ToMarkdown_Internal_UsesPlaceholder(t *testing.T) {
	...
	ctx := converter.ConversionContext{Registry: newTestRegistry(), PlaceholderManager: mgr}
...
func TestMediaSingleConverter_FromMarkdown(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mc := NewMediaSingleConverter()
			node, consumed, err := mc.FromMarkdown([]string{tt.line}, 0, converter.ConversionContext{Registry: newTestRegistry()})
			if tt.wantErr {
				require.Error(t, err)
				return
			}
			...
			// mediaSingle wrapper
			assert.Equal(t, adf_types.NodeTypeMediaSingle, node.Type)
			assert.Equal(t, tt.wantLayout, node.Attrs["layout"])

			// media child
			require.Len(t, node.Content, 1)
			media := node.Content[0]
			assert.Equal(t, adf_types.NodeTypeMedia, media.Type)
...
func TestMediaSingleConverter_FromMarkdown_EmptyLines(t *testing.T) {
	mc := NewMediaSingleConverter()
	_, _, err := mc.FromMarkdown([]string{}, 0, converter.ConversionContext{Registry: newTestRegistry()})
	require.Error(t, err)
}

func TestMediaSingleConverter_FromMarkdown_StartIndexOutOfBounds(t *testing.T) {
	mc := NewMediaSingleConverter()
	_, _, err := mc.FromMarkdown([]string{"![alt](url)"}, 5, converter.ConversionContext{Registry: newTestRegistry()})
	require.Error(t, err)
}
...
func TestMediaSingleConverter_CanHandle(t *testing.T) {
	mc := NewMediaSingleConverter()
	assert.True(t, mc.CanHandle(converter.ADFNodeType(adf_types.NodeTypeMediaSingle)))
	assert.False(t, mc.CanHandle(converter.ADFNodeType(adf_types.NodeTypeMediaInline)))
	assert.False(t, mc.CanHandle(converter.ADFNodeType("paragraph")))
}

func TestMediaSingleConverter_GetStrategy(t *testing.T) {
	mc := NewMediaSingleConverter()
	assert.Equal(t, converter.StandardMarkdown, mc.GetStrategy())
}

func TestMediaSingleConverter_ValidateInput(t *testing.T) {
	mc := NewMediaSingleConverter()

	assert.NoError(t, mc.ValidateInput(adf_types.ADFNode{Type: adf_types.NodeTypeMediaSingle}))
	assert.Error(t, mc.ValidateInput(adf_types.ADFNode{Type: "paragraph"}))
...
func TestMediaSingleConverter_Roundtrip_ExternalCenter(t *testing.T) {
	...
	// ADF → MD
	md, err := mc.ToMarkdown(original, converter.ConversionContext{Registry: newTestRegistry()})
	require.NoError(t, err)
	assert.Equal(t, "![alt text](https://example.com/img.png)\n\n", md.Content)

	// MD → ADF
	line := strings.TrimSpace(md.Content)
	restored, _, err := mc.FromMarkdown([]string{line}, 0, converter.ConversionContext{Registry: newTestRegistry()})
	require.NoError(t, err)
