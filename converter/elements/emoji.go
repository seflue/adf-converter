...
func NewEmojiConverter() element.Converter {
...
func (c *emojiConverter) ToMarkdown(node adf_types.ADFNode, context element.ConversionContext) (element.EnhancedConversionResult, error) {
	if node.Type != adf_types.NodeTypeEmoji {
		return element.EnhancedConversionResult{}, fmt.Errorf("expected emoji node, got %s", node.Type)
	}
	...
	if emojiChar == "" {
		return element.EnhancedConversionResult{}, fmt.Errorf("emoji node missing shortName attribute")
	}
...
func (c *emojiConverter) FromMarkdown(lines []string, startLine int, context element.ConversionContext) (adf_types.ADFNode, int, error) {
...
func (c *emojiConverter) CanHandle(nodeType element.ADFNodeType) bool {
	return nodeType == adf_types.NodeTypeEmoji
}

// GetStrategy returns the conversion strategy for emoji nodes
func (c *emojiConverter) GetStrategy() element.ConversionStrategy {
