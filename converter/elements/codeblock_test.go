...
func TestCodeBlock_ToMarkdown_WithLanguage(t *testing.T) {
	...
	result, err := cb.ToMarkdown(node, converter.ConversionContext{Registry: newTestRegistry()})
...
func TestCodeBlock_FromMarkdown_WithLanguage(t *testing.T) {
	...
	node, consumed, err := cb.FromMarkdown(lines, 0, converter.ConversionContext{Registry: newTestRegistry()})
	require.NoError(t, err)
...
func TestCodeBlock_FromMarkdown_LinesConsumed(t *testing.T) {
	...
	_, consumed, err := cb.FromMarkdown(lines, 0, converter.ConversionContext{Registry: newTestRegistry()})
	require.NoError(t, err)
	assert.Equal(t, 3, consumed)
}
...
func TestCodeBlock_FromMarkdown_SpecialLanguages(t *testing.T) {
	...
	for _, lang := range languages {
		t.Run(lang, func(t *testing.T) {
			...
			node, _, err := cb.FromMarkdown(lines, 0, converter.ConversionContext{Registry: newTestRegistry()})
			require.NoError(t, err)
...
func TestCodeBlock_FromMarkdown_UnclosedFence(t *testing.T) {
	...
	_, _, err := cb.FromMarkdown(lines, 0, converter.ConversionContext{Registry: newTestRegistry()})
	assert.Error(t, err)
}
...
func TestCodeBlock_FromMarkdown_StartIndex(t *testing.T) {
	...
	node, consumed, err := cb.FromMarkdown(lines, 1, converter.ConversionContext{Registry: newTestRegistry()})
	require.NoError(t, err)
	assert.Equal(t, 3, consumed)
	assert.Equal(t, "go", node.Attrs["language"])
	assert.Equal(t, "code", node.Content[0].Text)
...
func TestCodeBlock_RoundTrip_Simple(t *testing.T) {
	...
	// ADF -> Markdown
	result, err := cb.ToMarkdown(original, converter.ConversionContext{Registry: newTestRegistry()})
	require.NoError(t, err)

	// Markdown -> ADF
	lines := strings.Split(strings.TrimSuffix(result.Content, "\n\n"), "\n")
	restored, _, err := cb.FromMarkdown(lines, 0, converter.ConversionContext{Registry: newTestRegistry()})
	require.NoError(t, err)
