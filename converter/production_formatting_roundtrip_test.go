...
3. Third item`
	...
	adf, err := converter.FromMarkdownLegacy(productionMarkdown, session)
	...
	secondADF, err := converter.FromMarkdownLegacy(roundtripMarkdown, session)
...
func countExpandElements(adfDoc adf_types.ADFDocument) int {
	count := 0
...
func countExpandInNode(node adf_types.ADFNode) int {
	count := 0
...
func validateFormattingMarks(t *testing.T, adfDoc adf_types.ADFDocument) {
	var issues []string
...
func traverseAndValidateMarksInNode(node adf_types.ADFNode, issues *[]string) {
	...
	if node.Type == adf_types.NodeTypeText {
		...
		if strings.Contains(node.Text, "italic") && strings.Contains(node.Text, "_") {
			...
			for _, mark := range node.Marks {
				if mark.Type == adf_types.MarkTypeEm {
					hasEmMark = true
					break
				}
			}
...
func validateJiraAPICompatibility(adfDoc adf_types.ADFDocument) error {
	var errors []string
...
func validateJiraStructureInNode(node adf_types.ADFNode, errors *[]string, expandTitles map[string]int, depth int) {
...
func TestSpecificFormattingIssues(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			adf, err := converter.FromMarkdownLegacy(tc.markdown, session)
